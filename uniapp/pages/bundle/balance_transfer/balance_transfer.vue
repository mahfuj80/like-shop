  ...
  <view class="balance-transfer">
    <view class="contain bg-white">
      <input
        ...
        v-model="userSn"
        type="text"
        :placeholder="$t('请输入对方会员ID/手机号码进行转账')"
      />
    </view>
    <view class="contain transfer-money bg-white mt20">
      <view class="muted xs">{{ $t("转账金额") }}</view>
      <view class="input row-center">
        ...
        <view class="column" style="flex: none">
          ...
          <view class="xs mt10" style="color: #bbbbbb"
            >{{ $t("钱包余额") }}￥{{ wallet.user_money }}</view
          >
        </view>
      </view>
      <view class="btn lg white row-center br60" @tap="transferBtn">{{
        $t("转账")
      }}</view>
      <navigator
    ...
    <view class="user contain bg-white mt20" v-if="transferList.length">
      <view class="lg bold">{{ $t("最近转账") }}</view>
      <view class="list">
        <view
          class="item row"
        ...
        >
          ...
          <view>
            ...
            <view class="xs muted">ID:{{ item.sn }}</view>
          </view>
        </view>
      </view>
    </view>
      ...
      <view class="slot-content row-center" style="padding: 40rpx 80rpx">
        <view class="transfer-user row">
          ...
          <view>
            ...
            <view class="xs muted mt10">ID:{{ transferInfo.sn }}</view>
          </view>
        </view>
      </view>
    </u-modal>
