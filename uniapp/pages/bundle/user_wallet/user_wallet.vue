    ...
    <view class="contain bg-white mb20">
      <view class="header">
        <view class="white mb20">
          <view class="xs">{{ $t("总资产") }}</view>
          <view style="font-size: 76rpx">{{ wallet.user_money }}</view>
        </view>
        <view class="money white row">
          <view class="item">
            <view class="xs">{{ $t("累计充值") }}</view>
            <view style="font-size: 38rpx">{{
              wallet.total_recharge_amount || 0
            }}</view>
          </view>
          <view class="item">
            <view class="xs">{{ $t("累计消费") }}</view>
            <view style="font-size: 38rpx">{{
              wallet.total_order_amount || 0
            }}</view>
          </view>
          <navigator
            v-if="wallet.open_racharge"
            style="height: 58rpx"
            hover-class="none"
            url="/pages/bundle/user_payment/user_payment"
            class="row-center primary bg-white br60 btn"
            size="xs"
            >{{ $t("充值") }}</navigator
          >
        </view>
      ...
      <view class="nav row">
        <navigator
          ...
          hover-class="none"
          url="/pages/bundle/balance_transfer/balance_transfer"
        >
          <image src="/static/images/icon_yezz.png"></image>
          <view class="mt10">{{ $t("余额转账") }}</view>
        </navigator>
        <navigator
          class="nav-item column-center"
          hover-class="none"
          url="/pages/bundle/user_bill/user_bill?type=0"
        >
          <image src="/static/images/icon_zhmx.png"></image>
          <view class="mt10">{{ $t("账户明细") }}</view>
        </navigator>
        ...
        >
          ...
          <view class="mt10">{{ $t("转账记录") }}</view>
        </navigator>
        <navigator
          class="nav-item column-center"
          hover-class="none"
          url="/pages/bundle/recharge_code/recharge_code"
        >
          <image src="/static/images/icon_czjl.png"></image>
          <view class="mt10">{{ $t("充值记录") }}</view>
        </navigator>
        <navigator
          class="nav-item column-center"
          hover-class="none"
          url="/pages/bundle/user_withdraw/user_withdraw"
        >
          <image src="/static/images/icon_czjl.png"></image>
          <view class="mt10">{{ $t("提现") }}</view>
        </navigator>
      </view>
      <!-- <view class="activity">
