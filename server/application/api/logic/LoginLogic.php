...
class LoginLogic extends LogicBase
{
    public static function register($post)
    {
        ...
        if (!empty($post['pcode'])) {
            $pUserInfo = User::where('distribution_code', $post['pcode'])->find();
            if (empty($pUserInfo['id'])) {
                throw new SystemException('邀请码错误或者失效!');
            }
            $user_data['levelp'] = $pUserInfo['levelp'] + 1;
            $user_data['pids'] = $pUserInfo['pids'] . $pUserInfo['id'] . ',';
        }

        // 上级用户
        $user_data['nickname'] = $user_data['sn'];
        ...
        if (!empty($post['pcode'])) {
            // 生成邀请关系
            $result = DistributionLogic::code($post['pcode'], $user->id);
            if ($result !== true) {
                throw new SystemException('用户注册成功, 但是推荐关系生成失败!');
            }
        }
        return $token;
    }
    ...
    public static function login($post)
    {
        ...
        if (!empty($user_info['disable'])) {
            throw new SystemException('用户被禁用, 不可登录!');
        }
        return $user_info;
    }
