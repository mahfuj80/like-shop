<template>
  <view class="goods-details">
    ...
    <view class="contain" v-if="!isNull">
      ...
      <view class="seckill row-between" v-if="goodsType == 1">
        ...
        <view class="down column-center">
          <view class="xxs primary mb10">{{ $t("距活动结束仅剩") }}</view>
          <u-count-down
            :timestamp="countTime"
          ...
          <view class="row-between ml20 white" style="flex: 1">
            <view style="align-items: baseline" class="row">
              ...
              <price-format
                ...
                :first-size="46"
                :second-size="32"
                :price="checkedGoods.team_price || team.team_min_price"
                :weight="500"
              ></price-format>
            </view>
            <view class="mr20 row group-num">
              <view class="group-icon">
                <image
                  src="/static/images/icon_group.png"
                  class="icon-sm"
                ></image>
              </view>
      ...
      <view class="goods-info bg-white">
        ...
        >
          <view class="vip-price row">
            <view class="price-name xxs">{{ $t("会员价") }}</view>
        ...
        <view class="row-between xs lighter" style="padding: 0 24rpx 20rpx">
          <text v-if="goodsDetail.stock !== true"
            >{{ $t("库存") }}:
            {{ checkedGoods.stock || goodsDetail.stock }}</text
          >
          <text> {{ $t("销量") }} : {{ goodsDetail.sales_sum }}</text>
          <text> {{ $t("浏览量") }} : {{ goodsDetail.click_count }}</text>
        </view>
      </view>
      ...
      <view
        ...
        @tap="showSpecFun(0)"
      >
        <view class="text lighter">{{ $t("已选") }}</view>
        <view class="line1 mr20" style="flex: 1">{{
          $t(checkedGoods.spec_value_str) || $t("默认")
        }}</view>
      ...
      <navigator
        class="mt20"
        hover-class="none"
        url="/pages/bundle/server_explan/server_explan?type=2"
      >
        <view class="row bg-white" style="padding: 24rpx 24rpx">
          <view class="text lighter flex1">{{ $t("售后保障") }}</view>
          <image class="icon-sm" src="/static/images/arrow_right.png"></image>
        </view>
      </navigator>
      <view class="evaluation bg-white mt20">
        ...
        >
          <view>
            <text class="balck md mr10">{{ $t("用户评价") }}</text>
            <text class="primary sm"
              >{{ $t("好评率") }}{{ comment.goods_rate || "0%" }}</text
            >
          </view>
          <view class="row">
            <text class="lighter">{{ $t("查看全部") }}</text>
            <image class="icon-sm" src="/static/images/arrow_right.png"></image>
          </view>
        </navigator>
        ...
        <view class="con row-center muted" v-else>{{ $t("暂无评价") }}</view>
      </view>
      ...
      <view class="details mt20 bg-white">
        <view class="title lg">{{ $t("商品详情") }}</view>
        <view class="content">
          <u-parse
      ...
      <view class="footer row bg-white fixed">
        ...
        >
          ...
          <text class="xxs lighter">{{ $t("客服") }}</text>
        </navigator>
        <button
          class="btn column-center"
          hover-class="none"
        ...
        >
          ...
          <text class="xxs lighter">{{ $t("收藏") }}</text>
        </button>
        <navigator
        ...
        >
          ...
          <text class="xxs lighter">{{ $t("购物车") }}</text>
          <u-badge
            v-if="cartNum"
            bgColor="#FF2C3C"
            :offset="[8, 10]"
    ...
    <view
      class="share-money"
      :class="{
        show: showShare && goodsDetail.commission_price > 0 && goodsType == 0,
      }"
    >
      <view class="row-end">
