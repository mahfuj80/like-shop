      ...
      <view class="header">
        <view class="row">
          <u-image
            ...
            height="110rpx"
            shape="circle"
            :src="avatar == '' ? '/static/images/default_avatar.png' : avatar"
          />
          <view class="white ml20 row-between" style="flex: 1">
            <view>
              ...
              <view class="row">
                <navigator
                  class="sm row"
                  hover-class="none"
                  url="/pages/bundle/sign_rule/sign_rule"
                >
                  {{ $t("我的积分") }}
                  <image
                    src="/static/images/jifen_icon_help.png"
                    class="icon-sm ml10"
                  ></image>
                </navigator>
              </view>
            ...
            <navigator
              class="score-detail-entry row"
              url="/pages/bundle/sign_detail/sign_detail"
              hover-class="none"
            >
              <image
                style="
                  ...
                  margin-right: 7rpx;
                "
                src="/static/images/jifen_icon_data.png"
              ></image>
              <text class="sm white">{{ $t("积分明细") }}</text>
            </navigator>
          </view>
        </view>
      </view>
      <view class="main">
        <view class="contain bg-white">
          ...
          <view class="day-list row wrap">
            ...
            >
              <view
                :class="
                  'circle row-center ' +
                  (item.status == 1 ? 'active-circle' : '')
                "
              >
                <view class="num xs lighter" v-if="item.status != 1"
                  >+{{ item.integral }}</view
                >
                <image
                  class="num"
                  src="/static/images/jifen_icon_select.png"
                  v-if="item.status == 1"
                ></image>
              </view>
              <view class="day mt10 lighter sm"
                >{{ item.days }}{{ $t("天") }}</view
              >
            </view>
          </view>
          <view class="right-sgin">
            <button
              :class="
                'white br60 ' + (canSign == 1 ? 'gray' : 'primary-button')
              "
              @tap="userSignFun"
              size="md"
            >
              {{ canSign == 1 ? $t("已签到") : $t("立即签到") }}
            </button>
          </view>
        </view>
        <view class="contain bg-white mt20" v-if="makeInegral.length > 0">
          <view class="title row">
            ...
            <view class="bold xl">{{ $t("赚积分") }}</view>
          </view>
          <view class="task">
            <view
              v-for="(item, index) in makeInegral"
              :key="index"
              class="item row"
            >
              <image
                ...
                :src="
                  item.type == 1
                    ? '/static/images/icon_jifen_qiandao.png'
                    : item.type == 2
                    ? '/static/images/icon_jifen_pay.png'
                    : '/static/images/icon_jifen_invite.png'
                "
              ></image>
              <view class="con">
                <view class="md">{{ $t(item.name) }}</view>
                <view class="xs">
              ...
              <button
                hover-class="none"
                :class="'btn br60 ' + (item.status ? 'muted' : 'primary')"
                :style="
                  'border-color: ' + (item.status ? '#999999' : '#FF2C3C') + ';'
                "
                size="xs"
              >
                {{ item.status ? $t("已完成") : $t("未完成") }}
              </button>
            </view>
          </view>
        </view>
      ...
      <view class="pop-container">
        ...
        <view class="box column-center">
          <view class="desc row mt20 sm">
            <image
              style="width: 28rpx; height: 30rpx; margin-right: 8rpx"
              src="/static/images/icon_jifen.png"
            ></image>
            {{ addIntegral }}{{ $t("积分") }} + {{ addGrowth
            }}{{ $t("成长值") }}
          </view>
          <view class="bottom-box">
            <view class="md" style="line-height: 36rpx">
              {{ $t("您已连续签到") }}
              <text class="primary" style="font-size: 42rpx">{{
                signDays
              }}</text
              >{{ $t("天") }}
            </view>
        ...
        <view class="successlang">{{ $t("操作成功") }}!</view>
      </view>
    </u-popup>
  </view>
</template>
