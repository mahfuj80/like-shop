  ...
  <view class="sign-detail-container">
    <view v-for="(item, index) in detailList" :key="index">
      <view class="sign-item row-between">
        <view>
          <view class="nr sign-type">{{ $t(item.source_type) }}</view>
          <view class="muted xs mt10" style="line-height: 33rpx">{{
            item.create_time
          }}</view>
        </view>
        <view>
          <text class="primary lg" style="line-height: 45rpx">{{
            item.change_amount
          }}</text>
        </view>
      </view>
    ...
    <loading-footer :status="loadingStatus" slotEmpty>
      <view class="data-null column-center" slot="empty">
        ...
        <view class="muted sm">{{ $t("暂无其他记录") }}～</view>
      </view>
    </loading-footer>
  </view>
</template>
...
export default {
  ...
  methods: {
    getAccountLogFun() {
      let { detailList, loadingStatus, page } = this;

      loadingFun(getAccountLog, page, detailList, loadingStatus, {
        source: 2,
      }).then((res) => {
        if (res) {
          this.page = res.page;
