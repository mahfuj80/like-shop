        ...
        <view class="main">
          <view class="article-list">
            <view
              ...
              :key="index"
              :data-id="item.id"
              class="article-item bg-white"
              @tap="goPage"
            >
              <view class="row">
                <view class="info">
                  <view class="title lg line2 mb20">{{ $t(item.title) }}</view>
              ...
              <view class="row-between mt20">
                <view class="xs muted"
                  >{{ $t("发布时间") }}: {{ item.create_time }}</view
                >
                <view class="row">
                  <image
                    class="icon-sm"
                    src="/static/images/icon_see.png"
                  ></image>
                  <view class="ml10 xs muted"
                    >{{ item.visit }} {{ $t("人浏览") }}</view
                  >
                </view>
              </view>
            </view>
...
export default {
  ...
  methods: {
    ...
    getArticleListFun() {
      let { active, page, newsList, status } = this;
