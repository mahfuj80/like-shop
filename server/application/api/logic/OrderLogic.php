...
class OrderLogic extends LogicBase
{
    ...
    public static function info($post, $user_id)
    {
        try {
            ...
            foreach ($goods as $good) {
                ...
                if (apiUseEn()) {
                    $goods_info['goods_name'] = empty($goods_info['goods_name_en']) ? $goods_info['goods_name'] : $goods_info['goods_name_en'];
                }
    ...
    public static function goodsTemolate($infos, $item_id, $goods_num, $seckill_goods, $level, $post)
    {
        ...
        //会员折扣价格,不参与商品活动
        $level_discount = UserLevel::where('id', 1)->value('discount');
    ...
    {
        $field = 'i.id as item_id,g.id as goods_id,g.name as goods_name, g.name_en as goods_name_en,g.status,g.del,g.image,
    ...
    {
        $userinfo = fuserInfo($user_id);
        $goods_lists = $data['goods_lists'];
        $zhe = 0;
        $zhe_text = '';
        $old_order_amount = $data['order_amount'];
        if (!empty($userinfo['is_storer'])) {
            // 如果是店长直接打6折
            if ($userinfo['is_storer'] == 1) {
                $data['order_amount'] = bcmul($data['order_amount'], "0.65", 2);
                $zhe = 6.5;
                $zhe_text = '店长优惠';
                $data['dis_remark'] = '店长优惠' . $zhe . '折';
                $data['dis_zhe_money'] = bcsub($old_order_amount, $data['order_amount'], 2);
            }
            if ($userinfo['is_storer'] == 2) {
                $data['order_amount'] = bcmul($data['order_amount'], "0.6", 2);
                $zhe = 6;
                $zhe_text = '副店长优惠';
                $data['dis_remark'] = '副店长优惠' . $zhe . '折';
                $data['dis_zhe_money'] = bcsub($old_order_amount, $data['order_amount'], 2);
            }
        } else {
            // 如果是VIP, 进行打7折, 元, 除报单产品 FIXME: 使用自带的打折
            // $isBaodan = null;
            // foreach ($goods_lists as $_value) {
            //     $goodis_id = $_value['item_id'];
            //     if ($isBaodan === null) {
            //         $isBaodan = checkIsBaoDan($goodis_id);
            //     }
            // }
            // if (!empty($userinfo['level']) && !$isBaodan) {
            //     $data['order_amount'] = bcmul($data['order_amount'], "0.7", 2);
            //     $zhe = 7;
            //     $zhe_text = 'VIP会员优惠';
            //     $data['dis_remark'] = 'VIP会员' . $zhe . '折优惠';
            //     $data['dis_zhe_money'] = bcsub($old_order_amount, $data['order_amount'], 2);
            // }
        }
        ...
        try {
            ...
            if ($data['pay_way'] == Pay::BALANCE_PAY) {
                ...
                if ($user_money < $data['order_amount']) {
                    throw new Exception('账户余额不足:' . $user_money . ', 订单实际需要付款金额: ' . $data['order_amount']);
                }
            }
            ...
            // ! INFO: 如果是线下支付, 只标记订单状态为待上传凭证
            // if ($data['pay_way'] == Pay::OFFLINE_PAY) {
            //     PayNotifyLogic::handle('orderoff', $order['order_sn'], []);
            // }
    ...
    public static function addOrder($user_id, $data, $order_source, $user_address, $extra = [])
    {
        ...
        $order_data = [
            ...
            'dis_zhe_money' => $data['dis_zhe_money'] ?? 0,
        ];
    ...
    public static function addOrderGoods($order_id, $goods_lists)
    {
        foreach ($goods_lists as $k1 => $good) {
            ...
            $goods_data[] = [
                ...
                'goods_name_en' => $good['goods_name_en'] ?? null,
    ...
    public static function getOrderList($user_id, $type, $page, $size)
    {
        ...
        $lists = $order->where(['del' => 0, 'user_id' => $user_id])
            ->where($where)
            ->with(['orderGoods'])
            ->field('id,order_sn,order_status,pay_status,order_amount,order_status,order_type,shipping_status,create_time, pay_way, pay_img, offline_status')
            ->page($page, $size)
            ->order('id desc')
            ->select();

        $lists->append(['goods_count', 'pay_btn', 'img_btn', 'cancel_btn', 'delivery_btn', 'take_btn', 'del_btn', 'comment_btn', 'order_cancel_time']);
        ...
        foreach ($lists as $list) {
            foreach ($list['order_goods'] as &$order_goods) {
                ...
                if (apiUseEn()) {
                    $order_goods['goods_name'] = empty($order_goods['goods_name_en']) ? $order_goods['goods_name'] : $order_goods['goods_name_en'];
                }
            }
        }
    ...
    public static function getOrderDetail($order_id, $user_id)
    {
        ...
        foreach ($order->order_goods as &$item) {
            ...
            if (apiUseEn()) {
                $item['goods_name'] = !empty($item['goods_name_en']) ? $item['goods_name_en'] : $item['goods_name'];
            }
