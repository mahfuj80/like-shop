...
class Index extends AdminBase
{
    ...
    public function index()
    {
        ...
        if(isset($get['lang'])){
            if ($get['lang'] == 'en') {
                cookie('think_var', 'en');
                $this->assign('lang', $get['lang']);
            } else {
                cookie('think_var', 'zh-Hans');
                $this->assign('lang','zh');
            }
        } else {
            if (!empty(cookie('think_var'))) {
                $this->assign('lang',cookie('think_var'));
            }
        }
        ...
        foreach ($menu as $key => $value) {
            if($lang == 'en'){
                $menu[$key]['name'] = lang($value['name']);
            }
            foreach ($value['sub'] as $k => $v) {
                if($lang == 'en'){
                    $menu[$key]['sub'][$k]['name'] = lang($v['name']);

                }
                foreach ($v['sub'] as $kk => $vv) {
                    if($lang == 'en'){
                        $menu[$key]['sub'][$k]['sub'][$kk]['name'] = lang($vv['name']);
                    }
                }
            }
        }
