...
class Goods extends Model{
    ...
    public function Like(){
        $goods = new self();
        $like_list = $goods->where(['del'=>0,'is_like'=>1,'status'=>1])->field('id,name,name_en,image,min_price as price,market_price')->orderRaw('rand()')->limit(5)->select();
        if (!empty($like_list)) {
            foreach ($like_list as &$_value) {
                if (apiUseEn()) {
                    $_value['name'] = empty($_value['name_en']) ?  $_value['name'] :  $_value['name_en'];
                }
            }
        }
