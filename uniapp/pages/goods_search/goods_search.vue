      ...
      <view v-if="hotList.length" class="search-words">
        ...
        <view class="words row wrap">
          <view
            ...
            :key="index"
            class="item br60 mr20 mb20 lighter sm line1"
            @tap="onChangeKeyword(item)"
            >{{ item }}</view
          >
        </view>
      </view>
...
import { trottle, loadingFun, getRect } from "@/utils/tools";
...
export default {
  ...
  methods: {
    ...
    async getGoodsSearchFun() {
      let { page, goodsList, keyword, priceSort, saleSort, status } = this;
