...
class UserLevelLogic
{
    ...
    public static function lists($id)
    {
        ...
        // ->field('id,name,growth_value,image,background_image,privilege')
            ->select();
        $mylevel = Db::name('user_level')->where('id', 1)->find();
        ...
        if ($give_growth > 0) {
            $rule .= '使用余额充值，每元赠送' . $give_growth . "成长值。" . PHP_EOL;
        }
        $rule .= '';
        ...
        $list = [
            ...
            'growth_rule' => '',
            'mylevel' => $mylevel,
            'check_has_applay' => Db::name("user_level_log")
                ->where('uid', $id)
                ->where('status', 0)
                ->count(),
        ];
