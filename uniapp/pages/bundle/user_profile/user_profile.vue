  ...
  <view class="user-profile-container mt10">
    <view class="user-profile">
      <view class="user-avatar-box column-center">
        <uploader @after-read="afterRead" useSlot>
          <image
            ...
            :src="
              userInfo.avatar ||
              require('../../../static/images/default_avatar.png')
            "
          />
        </uploader>

        <!-- #ifdef MP-WEIXIN -->
        <view class="xs muted row-center" @click="getUserProfileFun"
          >更新微信头像</view
        >
        <!-- #endif -->
      </view>
      <view class="row-info row bdb-line" @click="changeName">
        <view class="label md">{{ $t("昵称") }}</view>
        <view class="md row" style="flex: 1">{{ userInfo.nickname }}</view>
        <u-icon name="arrow-right" />
      </view>
      <view class="row-info row bdb-line" @click="changeSex()">
        <view class="label md">{{ $t("性别") }}</view>
        <view
          class="md row"
          :class="userInfo.sex == 0 ? 'muted' : ''"
          style="flex: 1"
        >
          {{
            userInfo.sex == 0
      ...
      <view class="row-info row bdb-line">
        <view class="label md">{{ $t("手机号") }}</view>
        <view
          class="md row"
        ...
        >
          {{ userInfo.mobile ? $t("更换手机号") : $t("绑定手机号") }}
        </button>
        <!-- #endif -->
      </view>
      <!-- #ifndef MP-WEIXIN -->
      <view class="row-info row-between" @click="showPwdPop">
        <view class="label md">{{ $t("登录密码") }}</view>
        <view class="row">
          <view class="muted nr">{{ $t("点击设置") }}</view>
          <u-icon name="arrow-right" />
        </view>
      </view>
      <!-- #endif -->
      <view class="row-info row-between bdb-line" @tap="toSetPayPwd">
        <view class="label md">{{ $t("交易密码") }}</view>
        <u-icon name="arrow-right" />
      </view>
      <view class="row-info row-between bdb-line mt10" @click="goToExplain(0)">
        <view class="label md">{{ $t("服务协议") }}</view>
        <u-icon name="arrow-right" />
      </view>
      <view class="row-info row-between bdb-line" @click="goToExplain(1)">
        <view class="label md">{{ $t("隐私协议") }}</view>
        <u-icon name="arrow-right" />
      </view>
      <view class="row-info row-between">
        <view class="label md">{{ $t("关于我们") }}</view>
        <view>v0.9</view>
      </view>
      <!-- <view class="row-info row bdb-line" @click="changeLang()">
        <view class="label md">{{ $t("语言") }}</view>
        <view class="md row" style="flex: 1">
        </view>
        <u-icon name="arrow-right" />
      </view> -->
      <!-- #ifndef MP-WEIXIN -->
      <view class="bg-primary white save-btn row-center lg" @click="logout">{{
        $t("退出登录")
      }}</view>
      <!-- #endif -->
    </view>
    ...
    >
      <view class="modify-container column-center bg-white" v-show="showMobile">
        ...
        >
        <view class="btn bg-primary white row-center" @click="$changeUserMobile"
          >确定</view
        >
      </view>
    </u-popup>
    ...
    >
      <view
        ...
        v-show="showNickName"
      >
        <view class="title xl">{{ $t("修改用户名") }}</view>
        <view class="modify-row row">
          <view style="width: 71px">{{ $t("新昵称") }}</view>
          <input v-model="newNickname" :placeholder="$t('请输入新的昵称')" />
        </view>
        <view
    ...
    <u-popup v-model="showPwd" closeable mode="center" border-radius="14">
      <view class="modify-container column-center bg-white" v-show="showPwd">
        <view class="title xl">{{ $t("设置密码") }}</view>
        <view class="modify-row row">
          <view style="width: 56px; border-right: 1px solid #e5e5e5"></view>
          <view style="margin-left: 15px">{{ userInfo.mobile }}</view>
        </view>
        <!-- <view class="modify-row row">
        ...
        <view class="modify-row row">
          <view style="width: 71px">{{ $t("设置密码") }}</view>
          <input
            type="password"
            v-model="pwd"
        ...
        <view class="modify-row row">
          <view style="width: 71px">{{ $t("确认密码") }}</view>
          <input
            type="password"
            v-model="comfirmPwd"
            :placeholder="$t('再次输入新密码确认')"
          />
        </view>
        <view class="btn bg-primary white row-center" @click="$forgetPwd">{{
          $t("确定")
        }}</view>
      </view>
    </u-popup>
    ...
    <!-- <view class="xs muted" style="margin: 50rpx 0">
      <view class="row-center"> 由 likeshop 提供免费开源商城系统 </view>
      <view class="row-center"> © likeshop.cn </view>
    </view> -->
  </view>
</template>
...
import { HTTP_URL, ACCESS_TOKEN } from "@/config/app";
...
export default {
  ...
  methods: {
    ...
    showModifyMobile() {
      ...
      this.smsType = this.userInfo.mobile
  ...
  computed: {
    ...mapState(["token"]),
    sexList() {
      return [this.$t("男"), this.$t("女")];
    },
    locales() {
      return [this.$t("en"), this.$t("zh-Hans")];
    },
  },
};
</script>
