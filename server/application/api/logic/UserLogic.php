...
class UserLogic extends LogicBase
{
    public static function center($user_id)
    {
        ...
        $user['next_level_num'] = '';
        if ($user_level) {
            $_num = intval($user_level['growth_value'] - $user['user_growth']);
            $user['next_level_tips'] = '距离升级还差' . $_num;
            $user['next_level_num'] = $_num;
        }
        // 是否设置支付密码
        ...
        $user = $user->toArray();
        $user['today_y'] = (function () use ($user_id) {
            return AccountLog::where('user_id', $user_id)
                ->where('change_type', 1)
                ->where('source_type', 'in', [
                    AccountLog::order_p1,
                    AccountLog::order_p2,
                    AccountLog::order_p3,
                    AccountLog::order_team,
                    AccountLog::order_teamall,
                ])->whereTime('create_time', 'today')
                ->sum('change_amount');
        })();
        $user['today_j'] = (function () use ($user_id) {
            return AccountLog::where('user_id', $user_id)
                ->where('change_type', 1)
                ->where('source_type', 'in', [
                    AccountLog::order_p1,
        ...
        $user['week_y'] = (function () use ($user_id) {
            return AccountLog::where('user_id', $user_id)
                ->where('change_type', 1)
                ->where('source_type', 'in', [
                    AccountLog::order_p1,
        ...
        $user['week_j'] = (function () use ($user_id) {
            return AccountLog::where('user_id', $user_id)
                ->where('change_type', 1)
                ->where('source_type', 'in', [
                    AccountLog::order_p1,
