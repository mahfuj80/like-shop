...
<view class="user-spread">
	<view class="explain row muted" v-if="vipState==0 || vipState==1">
	    ...
	    <span class="ml10">{{  $t('成为分销会员，推广下级可获得额外收益，推广越多收益越多')  }}</span>
	</view>
    ...
    <view class="header" v-if="!showLoading">
        <view class="user-info row">
            <view class="user-avatar">
                ...
                <view class="avatar-tag white" v-if="vipState==2">{{ $t('分销会员') }}</view>
            </view>
            <view class="user-message-container column ml20 white">
                ...
                <view class="xs row" style="line-height: 30rpx;text-align: left;">
                    {{ $t('我的邀请人') }}：{{!inviteStatus ?  $t('无')  : userInfo.leader.nickname}} <view v-if="!inviteStatus" class="br60 white write-btn row-center" @tap="showInvitePop">{{ $t('填写') }}</view>
                </view>
            </view>
        </view>
    </view>
                ...
                <view class="user-apply-vip column-center">
                    ...
                    <view class="vip-form">
                        <view class="form-item row">
                            <view class="label md">{{ $t('真实姓名') }}：</view>
                            <input class="md" name="real_name" v-model="realName" :placeholder=" $t('请输入您的真实姓名') "></input>
                        </view>
        ...
        <view :hidden="vipState!=2" class="user-vip">
            <view class="user-assets-box">
                <view class="user-assets-header row-between">
                    ...
                    <navigator hover-class="none" class="primary-btn white row-center" url="/pages/bundle/user_withdraw/user_withdraw">{{ $t('立即提现') }}</navigator>
                </view>
                <view class="user-assets-content row wrap">
                    <view class="user-item column-center">
                        ...
                        <view class="nr user-assets-name row" style="color: #8F430E">
                            {{ $t('今日预估收益') }}   <!-- <u-icon class="ml10" name="question-circle" size="30rpx" color="#D88D5A" /> -->
                        </view>
                    ...
                    <view class="user-item column-center">
                        <view class="nr user-assets-name row" style="color: #8F430E">
                            {{ $t('累计获得收益') }}<!-- <u-icon class="ml10" name="question-circle" size="30rpx" color="#D88D5A" /> -->
                        </view>
                        <view class="assets">
            ...
            <view class="mt20 fans-msg-box row bg-white md">
                <navigator url="/pages/user/team/team" hover-class="none" class="my-fans row-center normal">
                    {{ $t('我的粉丝') }} <span class="primary ml10">{{userFans}}</span>
                    <u-icon class="ml10" name="arrow-right" size="28rpx" color="#666" />
                </navigator>
                <view class="line"></view>
                <navigator url="/pages/user/invite/index" hover-class="none" class="invite-fans row-center normal">
                    {{ $t('邀请粉丝') }}
                    <u-icon class="ml10" name="arrow-right" size="28rpx" color="#666" />
                </navigator>
            </view>
            <view class="my-invite-box mt20 bg-white column-center">
                <view class="my-invite-title sm normal">{{ $t('我的邀请码') }}</view>
                <view class="row bold mt20" style="font-size: 42rpx;line-height: 30rpx">
                    {{userInfo.user.distribution_code}} <view class="invite-copy-btn ml10 xxs" @click="onCopy">{{ $t('复制') }}</view>
                </view>
...
export default {
  ...
  methods: {
    ...
    formSubmit(e) {
      let { provinceId, cityId, districtId, reason } = this;
