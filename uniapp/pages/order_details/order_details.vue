			...
			<view class="main">
				<view class="header">
					<view class="item" v-if="orderDetail.order_status == 0">
						<view class="white lg mb10">{{ $t('等待买家付款') }}</view>
						<view class="white sm row" v-if="cancelTime > 0">{{ $t('支付剩余') }} <u-count-down separator="zh" :show-hours="false"
							 :show-seconds="false" :timestamp="cancelTime" separator-color="#fff" color="#fff" :separator-size="26"
							 :font-size="26" bg-color="transparent"></u-count-down> {{ $t('自动关闭') }}</view>
					</view>
					<view class="item" v-if="orderDetail.order_status == 1">
						<view class="white lg mb10">{{ $t('等待商家发货') }}</view>
						<view class="white sm">{{ $t('您的商品正在打包中，请耐心等待') }}…</view>
					</view>
					<view class="item" v-if="orderDetail.order_status == 2">
						<view class="white lg mb10">{{ $t('已发货') }}</view>
						<view class="white sm">{{ $t('您的商品正在路中，请耐心等待') }}…</view>
					</view>
					<view class="item" v-if="orderDetail.order_status == 3">
						<view class="white lg mb10">{{ $t('已完成') }}</view>
						<view class="white sm">{{ $t('商品已签收，期待再次购买') }}！</view>
					</view>
					<view class="item" v-if="orderDetail.order_status == 4">
						<view class="white lg mb10">{{ $t('订单已关闭') }}</view>
				...
				<view class="price contain">
					...
					<view v-if="orderDetail.dis_zhe_money > 0" class="row-between">
						<view>{{ $t('vip或者店长折扣') }}</view>
						<view class="primary">-
							<price-format :price="orderDetail.dis_zhe_money"></price-format>
						</view>
					</view>
					<view class="row-end">
						<view class="lighter">{{ $t('实付金额') }}：</view>
				...
				<view class="footer bg-white row fixed" v-if="orderDetail.cancel_btn || orderDetail.delivery_btn || orderDetail.take_btn || orderDetail.del_btn || orderDetail.pay_btn">
					...
					<view v-if="orderDetail.cancel_btn">
						<button size="sm" class="plain br60" hover-class="none" @tap="cancelOrder">{{ $t('取消订单') }}</button>
					</view>
					<navigator v-if="false && orderDetail.delivery_btn" hover-class="none" :url="'/pages/bundle/goods_logistics/goods_logistics?id=' + orderDetail.id">
						<button size="sm" class="plain br60" hover-class="none">{{ $t('查看物流') }}</button>
					</navigator>
					<view v-if="orderDetail.take_btn" class="ml20">
						<button size="sm" class="plain br60 primary red" hover-class="none" @tap.stop="comfirmOrder">{{ $t('确认收货') }}</button>
					</view>
					<view v-if="orderDetail.del_btn">
						<button size="sm" class="plain br60" hover-class="none" @tap="delOrder">{{ $t('删除订单') }}</button>
					</view>
					<view class="ml20" v-if="orderDetail.pay_btn">
						<button size="sm" class="bg-primary br60 white" @tap="payNow">{{ $t('立即付款') }}</button>
					</view>
				</view>
			</view>
		</view>
