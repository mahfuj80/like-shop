    ...
    <view class="confirm-order">
      <view class="confirm-con">
        ...
        <view class="contain benefit" v-if="orderInfo.order_type == 0">
          <view class="item row-between" @tap="showCoupon = true">
            ...
            <view class="row">
              ...
              <text v-else class="muted">{{ $t("无优惠券可用") }}</text>
              <image
                class="icon-sm ml20"
          ...
          <view
            class="item row"
            @tap="changeIntegral"
            v-if="orderInfo.integral_switch"
          >
            <view>积分抵扣</view>
        ...
        <view class="pay-way contain">
          <view class="lighter title">{{ $t("支付方式") }}</view>
          <radio-group class="radio-group" @change="radioChange">
            ...
            >
              <view class="row">
                ...
                <view class>
                  <view class="balck bt10">{{ $t(item.name) }}</view>
        ...
        <view class="price contain">
          ...
          <view
            class="item row-between"
            v-if="orderInfo.zhe && orderInfo.zhe > 0"
          >
            <view>{{ $t(orderInfo.zhe_text) }}</view>
            <view class="primary">-{{ orderInfo.dis_zhe_money }}</view>
          </view>
        </view>
      </view>
    ...
    <u-popup v-model="showCoupon" border-radius="14" mode="bottom" closeable>
      ...
      <view v-if="showCoupon">
        <tabs :active="popActive" :config="{ underLineWidth: 100 }">
          <tab :title=" $t('可使用优惠券')  + '(' + usableCoupon.length + ')'">
            <coupon-obj
              :list="usableCoupon"
              :type="0"
          ...
          <tab :title=" $t('不可用优惠券')  + '(' + unusableCoupon.length + ')'">
            <coupon-obj
              :list="unusableCoupon"
              :type="1"
              @change="onSelectCoupon"
...
<script>
import { orderBuy, getOrderCoupon } from "@/api/order";
...
export default {
  ...
  methods: {
    ...
    changeIntegral() {
      const {
        useIntegral,
        orderInfo: { integral_limit, user_integral },
      } = this;
      if (integral_limit > user_integral)
        return this.$toast({
          title: this.$t("未满足使用条件"),
        });
    ...
    showDialog() {
      const { integral_config, integral_desc } = this.orderInfo;
      uni.showModal({
        title: this.$t("积分使用说明"),
      ...
      return new Promise((resolve) => {
        getOrderCoupon({
          ...
          if (res.code == 1) {
            ...
            (this.usableCoupon = usable), (this.unusableCoupon = unusable);
            resolve();
          }
        });
      });
    ...
    async orderBuyFun(action = "info") {
      let { address, userRemark, payWay, useIntegral, bargainLaunchId } = this;
      let submitObj = {
        action,
        goods: this.goods,
        pay_way: payWay,
      ...
      if (this.teamId) {
        const { item_id, num } = this.goods[0];
      ...
      const { data: orderData, code: orderCode } = this.teamId
        ? await teamBuy(submitObj)
        : await orderBuy(submitObj);
      if (orderCode != 1) return (this.showLoading = false);
      if (action == "info") {
        let { address, goods_lists } = orderData;
        ...
        let { order_id, type } = orderData;
        const { data: prepayData, code: prepayCode } = await prepay({
          from: type,
          order_id,
        });
        this.showLoading = false;
        ...
        } else if (prepayCode == 40001) {
          // 线下支付成功
          console.log(prepayData);
          this.payStatus = true;
          uni.redirectTo({
            url: `/pages/pay_result/pay_result?id=${prepayData}`,
          });
        }
      }
    },
