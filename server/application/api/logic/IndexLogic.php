    ...
    public static function lists($user_id){
        ...
        if($seckill){
            $seckill['goods'] = Db::name('goods g')
                ...
                ->limit(9)
                ->field('g.id,g.name,g.image,g.min_price,sg.price as seckill_price,sg.sales_sum,g.name_en')
                ->select();
            foreach ($seckill['goods'] as &$seckill_item ){
                ...
                if (apiUseEn()) {
                    $seckill_item['name'] = !empty($seckill_item['name_en']) ? $seckill_item['name_en'] : $seckill_item['name'];
                }
            }
        }else{
            $seckill['goods'] = [];
