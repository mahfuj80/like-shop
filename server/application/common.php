 ...
 * 前端用户验证并登录
 *
 * @param string $token $token = $request->header('token');
 * @return array
 * @copyright jcleng
 * @author jcleng
 */
function fusertokenLogin($token)
{
    //token验证
    $token_cache = new TokenCache($token, ['token' => $token]);
    //token缓存，直接通过
    if (!$token_cache->isEmpty()) {
        return $token_cache->get(600);
    }
    //token验证，并生成缓存
    $token_validate = new TokenValidate();
    $result = $token_validate->check(['token' => $token]);
    if ($result === true) {
        return $token_cache->set(600);
    }
    throw new SystemException('用户登录过期~!');
}

/**
 * 获取用户信息
 *
 * @param int|string $id_or_token id或者token
 * @return array 用户信息
 * @copyright jcleng
 * @author jcleng
 */
function fuserInfo($id_or_token)
{

    $user_info = Db::name('user u')
        ->join('session s', 'u.id=s.user_id');
    if (is_numeric($id_or_token)) {
        $user_info = $user_info->where(['u.id' => $id_or_token]);

    } else {
        $user_info = $user_info->where(['s.token' => $id_or_token]);
    }
    $user_info = $user_info->field('u.*,s.token,s.client')
        ->find();
    if (empty($user_info['id'])) {
        throw new SystemException('用户状态异常~!');
    }
    return $user_info;
}
 ...
 * @author jcleng
 */
function checkIsBaoDan($goods_id)
{
    return false;
}
...
{
    $fliename = Env::get('runtime_path') . $name . '.lock';
    $fp = fopen($fliename, 'a');
...
function apiUseEn() {
    $info = Request::header();
    if (!empty($info['accept-language']) && $info['accept-language'] == 'en') {
        return true;
    }
    if ($info['accept-language'] != 'zh-Hans') {
        return true;
    }
    return false;
}
