...
class GoodsLogic{
    ...
    public static function getGoodsList($user_id,$get,$page,$size){
        ...
        if(isset($get['is_vip']) && $get['is_vip'] == 1) {
            $where[] = ['is_member', '=', 1];
        }
        ...
        $goods_list = $goods
                    ...
                    ->page($page,$size)
                    ->order($order)
                    ->field('id,name, name_en,image,min_price as price,market_price,sales_sum+virtual_sales_sum as sales_sum,sort')
                ->select();
        if (!empty($goods_list)) {
            foreach ($goods_list as $key => &$value) {
                if (apiUseEn()) {
                    $value['name'] = !empty($value['name_en']) ? $value['name_en'] : $value['name'];
                }
            }
        }
    ...
    public static function getGoodsDetail($user_id,$id){
        ...
        if($goods){
            ...
            if($user_id) {
                ...
                //会员折扣
                $member_discount = Db::name('user_level')
                    ->where('id', 1)
                    ->value('discount');
                $price_array = [];
                //处理会员折扣价格
    ...
    public static function getBestList($page,$size){
        ...
        $goods_list = $goods
                    ->where(['del'=>0,'status'=>1,'is_best'=>1])
                    ->field('id,name,image,min_price as price,market_price,name_en')
