	...
	<view class="user-withdraw">
		<view class="user-tab-container">
			<tabs :active="active" :line-width="40" @change="onChange" :config="{itemWidth: 200}">
				<tab :title=" $t(item.name) " :name=" $t(item.value) " v-for="(item, index) in widthDrawWay" :key="index">
					<template v-if="item.value == 1 || item.value == 2">
						<view class="bg-white withdraw-container mt20">
							<view class="input row-center">
								...
								<view class="column" style="flex: none;">
									<view class="xs primary" style="text-align: right;" @tap="allWithdraw">{{ $t('全部提现') }}</view>
									<view class="xs" style="color: #BBBBBB">{{ $t('可提现余额') }}￥{{widthDrawConfig.able_withdraw}}
									</view>
								</view>
							</view>
							<view class="tips mt20 muted row xs" v-if="item.value == 2">
								{{ $t('提示：提现需扣除服务费') }}{{widthDrawConfig.poundage_percent}}%，{{ $t('请自行缴纳税款') }}
						...
						<navigator url="/pages/bundle/user_withdraw_code/user_withdraw_code" hover-class="none"
							class="mt20 nr lighter row-center">{{ $t('提现记录') }}</navigator>
					</template>
					...
					<template v-if="item.value == 5">
						<view class="bg-white form-container">
							<view class="input-item row md">
								<view class="input-label ">{{ $t('银行卡账号') }}</view>
								<input v-model="account" :placeholder=" $t('请输入银行卡账号') "></input>
							</view>
							<view class="input-item row md">
								<view class="input-label ">{{ $t('真实姓名') }}</view>
								<input v-model="realName" :placeholder=" $t('请输入真实姓名') "></input>
							</view>
							<view class="input-item row md">
								<view class="input-label ">{{ $t('提现银行') }}</view>
								<input v-model="bank" :placeholder=" $t('请输入提现银行') "></input>
							</view>
							<view class="input-item row md">
								<view class="input-label ">{{ $t('银行支行') }}</view>
								<input v-model="subbank" :placeholder=" $t('请输入银行支行') "></input>
							</view>
							<view class="input-item row md">
								<view class="input-label ">{{ $t('备注') }}</view>
								<input v-model="remark" :placeholder=" $t('备注') "></input>
							</view>
						</view>
						<view class="bg-white withdraw-container mt10">
							<view class="input row-center">
...
import { uploadFile, trottle } from "@/utils/tools";
        ...
        if (res.code == 1) {
          this.$toast(
            ...
            },
            {
              tab: 2,
              url:
                "/pages/bundle/widthdraw_result/widthdraw_result?id=" +
                res.data.id,
            }
          );
        }
