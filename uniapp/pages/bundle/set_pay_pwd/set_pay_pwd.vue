	...
	<view class="set-pwd">
		<view class="input-container bg-white">
			<view class="input-item row" v-if="hasPayPwd">
				<view class="input-label md normal">{{ $t('原密码') }}</view>
				<u-input  class="flex1" type="password" v-model="originPwd" :placeholder=" $t('请输入原来的交易密码') " />
			</view>
			<view class="input-item row">
				<view class="input-label md normal">{{ $t('交易密码') }}</view>
				<u-input class="flex1" type="password" v-model="setPwd" :placeholder="hasPayPwd ?  $t('请输入新的交易密码')  :  $t('请输入交易密码') " />
			</view>
			<view class="input-item row">
				<view class="input-label md normal">{{ $t('确认密码') }}</view>
				<u-input  class="flex1" type="password" v-model="comfirmPwd" :placeholder=" $t('请再次确认密码') " />
			</view>
			<!-- <navigator hover-class="none" url="/pages/bundle/forget_pay_pwd/forget_pay_pwd"
				class="row-end lighter xs mr20" v-if="hasPayPwd">{{ $t('忘记原密码') }}？</navigator> -->
