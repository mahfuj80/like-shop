<template>
	<view>
		<u-modal :value="showTips" ref="uModal" show-cancel-button :confirm-color="primaryColor" async-close
			:cancelText=" $t('取消') "
			:confirm-text=" $t('前往绑定') " @confirm="toSetting" @cancel="goBack">
			<view class="slot-content row-center" style="padding: 40rpx;">
				{{ $t('您暂未绑定手机号，请先绑定') }}
			</view>
		</u-modal>
		<u-modal ref="uModalSet" :value="!showTips && showSetPwd" :title=" $t('设置交易密码') " show-cancel-button
			:confirm-color="primaryColor" async-close :cancelText=" $t('取消') " :confirm-text=" $t('确定') " @confirm="onSetPwd" @cancel="goBack">
			<view class="slot-content" style="padding: 40rpx;">
				<view style="font-size: 42rpx;">{{mobile}}</view>
				<view class="mt20">
					<u-input type="password" v-model="setPwd" :border="true" :placeholder=" $t('请设置您的交易密码') "
						style="width: 100%" />
				</view>
				<view class="mt20">
					<u-input type="password" v-model="comfirmPwd" :border="true" :placeholder=" $t('再次确认交易密码') "
						style="width: 100%" />
				</view>
			</view>
		</u-modal>
