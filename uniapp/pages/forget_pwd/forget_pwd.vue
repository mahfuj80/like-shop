    ...
    <view class="forget-pwd-container">
        <view class="forget-input-container">
            <view class="input-item row">
                <view class="input-label md normal">{{ $t('手机号') }}</view>
                <input class="input" v-model="mobile" :placeholder="$t('请输入手机号码')" />
            </view>
            <view class="input-item row">
                <view class="input-label md normal"> {{ $t('短信验证码') }} </view>
                <input v-model="smsCode" :placeholder=" $t('请输入') " style="width: 3.8rem"/>
                <button class="bd-primary sm primary br60 row-center" @click="sendSmsFun()">
                    <view v-show="canSendSms" style="flex: none">{{ $t('获取验证码') }}</view>
                    <u-count-down
                    ref="countDown"
                    :show-days="false"
                    :timestamp="time"
            ...
            <view class="input-item row">
                <view class="input-label md normal">{{ $t('重置密码') }}</view>
                <input type="password" v-model="resetPwd" :placeholder=" $t('6-20位数字字母或符号组合') " />
            </view>
            <view class="input-item row">
                <view class="input-label md normal">{{ $t('确认密码') }}</view>
                <input type="password" v-model="comfirmPwd" :placeholder=" $t('再次输入新密码确认') " />
            </view>
        </view>
