...
class AccountLog extends Model
{
    ...
    const withdraw_dec = 403;
    const withdraw_asc = 404;

    const order_store_add = 500;
    const order_p1 = 501;
    const order_p2 = 502;
    const order_p3 = 503;
    const order_team = 504;
    const order_teamall = 505;
    ...
    public static function getAcccountDesc($from = true)
    {
        $desc = [
            ...
            self::order_store_add => '店长秒杀下单',
            self::order_p1 => '直推1代',
            self::order_p2 => '直推2代',
            self::order_p3 => '直推3代',
            self::order_team => '团队奖励',
            self::order_teamall => '股东分红',
            self::withdraw_dec => '提现',
            self::withdraw_asc => '退回',

        ];
        if ($from === true) {
            return $desc;
        }
        return lang($desc[$from]) ? lang($desc[$from]) : ($desc[$from] ?? '-');
    }
    //返回变动类型
